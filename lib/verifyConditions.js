...
const plugins = {
    "s3": "https://github.com/hypnoglow/helm-s3.git",
    "gs": "https://github.com/hayorov/helm-gcs.git"
}
...
module.exports = async (pluginConfig, context) => {
    ...
    if (requirePlugin(pluginConfig)) {
        try {
            await installPlugin(pluginConfig)
        } catch(error) {
        ...
        try {
            await verifyBucketCredentials(pluginConfig.registry);
        } catch (error) {
            errors.push('Could not login to Bucket. Wrong credentials?', error);
        }
    }
...
async function verifyBucketCredentials(registryUrl) {
    await execa(
        'helm',
...
function getBucketProvider(config) {
    return config.registry.substr(0, config.registry.indexOf('://'))
}
...
function requirePlugin(config) {
    return Object.keys(plugins).includes(getBucketProvider(config))
}

async function installPlugin(config) {
    await execa('helm', ['plugin', 'install', getBucketProvider(config)])
}
